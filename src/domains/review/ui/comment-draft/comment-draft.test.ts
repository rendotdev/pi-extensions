...
describe("CommentDraft", () => {
  ...
  it("keeps rapid input local until the deferred review sync runs", () => {
    ...
    Draft.update({ id: "comment", value: "Fast typing", onSync });

    expect(Draft.value({ id: "comment", fallback: "" })).toBe("Fast typing");
  ...
  it("applies an unsynchronized draft before finishing a review", () => {
    ...
    Draft.update({ id: "comment", value: "Unsynchronized draft", onSync: vi.fn() });

    const reviewWithDraft = Draft.applyToReview({ review });
