...
describe("FileSearch", () => {
  ...
  it("ranks a matching filename ahead of a path-only match", () => {
    expect(FileSearch.search({ files, query: "review server" })[0]?.location).toBe(
      "src/modules/review/server/server.ts",
    );
  });
  ...
  it("matches initials and compact filename queries", () => {
    ...
    expect(FileSearch.search({ files, query: "maintsx" })[0]?.location).toBe("src/web/main.tsx");
  });
  ...
  it("filters 50,000 indexed files within an interactive time budget", () => {
    ...
    const exactResult = FileSearch.search({
      files: largeFileSet,
      query: "component 49999",
    });
    const typoResult = FileSearch.search({
      files: largeFileSet,
