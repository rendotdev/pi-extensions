...
import type { ReviewJson, ReviewPayload, ReviewStatus } from "../../types/review.ts";
import { ReviewSinceLastStore } from "./since-last-store.ts";
...
describe("ReviewSinceLastStore", () => {
  it("uses the newest compatible diff review in the repository", async () => {
    ...
    await expect(
      new ReviewSinceLastStore().collect({
        root,
