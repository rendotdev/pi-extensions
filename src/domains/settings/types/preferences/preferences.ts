...
export function parseLgtmPreferences(params: {
  value: unknown;
  defaults: LgtmPreferences;
}): LgtmPreferences {
  ...
  const diffStyle = preferences.diffStyle ?? params.defaults.diffStyle;
  ...
  const lineWrap = preferences.lineWrap ?? params.defaults.lineWrap;
  if (typeof lineWrap !== "boolean") {
    throw new Error("lineWrap must be a boolean.");
  }
  const sidebarWidth = preferences.sidebarWidth ?? params.defaults.sidebarWidth;
  ...
  const fileExpansion = preferences.fileExpansion ?? params.defaults.fileExpansion;
  ...
  const fileExpansionOverridesValue =
    preferences.fileExpansionOverrides ?? params.defaults.fileExpansionOverrides;
