...
import type { ReviewSourceFile } from "./review.ts";
import { ReviewGrouping } from "./review-grouping.ts";
...
describe("ReviewGroupingClass", () => {
  it("preserves authored group and file order", () => {
    expect(
      ReviewGrouping.build({
        files,
        groups: [
  ...
  it("leaves reviews without authored groups flat", () => {
    expect(ReviewGrouping.build({ files, groups: undefined })).toBeUndefined();
  });
});
