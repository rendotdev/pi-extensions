...
import {
  SSHCommandEncoderClass,
  SSHControlConnectionClass,
  SSHGitRepositoryReaderClass,
} from "./ssh-git-review-platform.ts";

describe("SSHCommandEncoderClass", () => {
  it("single-quotes remote arguments and preserves apostrophes", () => {
    const Encoder = new SSHCommandEncoderClass({ maximumCommandLength: 10_000 }, {});
  ...
  it("rejects NUL bytes and oversized commands", () => {
    const Encoder = new SSHCommandEncoderClass({ maximumCommandLength: 30 }, {});
...
describe("SSHControlConnectionClass", () => {
  it("opens one control connection, reads a framed response, and cleans up", async () => {
    ...
    const Connection = new SSHControlConnectionClass(
      {},
      {
...
describe("SSHGitRepositoryReaderClass", () => {
  it("collects tracked and untracked remote changes without remote lgtm", async () => {
    ...
    const Encoder = new SSHCommandEncoderClass({ maximumCommandLength: 65_536 }, {});
    const Reader = new SSHGitRepositoryReaderClass(
      { fileConcurrency: 2, maximumFileBytes: 1_000_000 },
      {
  ...
  it("requires an absolute remote repository path", async () => {
    const Reader = new SSHGitRepositoryReaderClass(
      { fileConcurrency: 1, maximumFileBytes: 1_000 },
      {
        commandEncoder: new SSHCommandEncoderClass({ maximumCommandLength: 1_000 }, {}),
        connection: {
          open: vi.fn(),
          execute: vi.fn(),
          close: vi.fn(),
